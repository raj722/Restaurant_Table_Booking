...
function Navbar() {
  const { user, role, logout } = useAuth(); // Get user, role, and logout function from context
  ...
  return (
    <nav className="flex items-center justify-between p-4 text-white bg-gray-800">
      ...
      <ul className="flex justify-center flex-grow space-x-6 text-center">
        ...
        {role === "restaurant" && (
          <li><Link to={`/restaurant/:id${user?.id}`} className="hover:text-gray-300">Restaurant Page</Link></li>
        )}
