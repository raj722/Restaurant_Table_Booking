...
import { auth, db } from '../config/firebase';  // Firebase Auth & Firestore
...
const RestaurantSignupPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Store restaurant owner details in Firestore
      await setDoc(doc(db, 'RestaurantUser', user.uid), {
        owner_id: user.uid,
        name: formData.name,
        ...
        role: 'restaurant',
        // status: 'pending', // Removed pending status
        comment: 'Restaurant owner registered successfully', // Added comment
      });

      console.log("RestaurantUser document added:", user.uid);  // Debugging log
