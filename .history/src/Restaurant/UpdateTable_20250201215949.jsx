...
const TableCard = ({ table, onUpdateStatus, onDeleteTable, onReserveTable, onToggleSelection, isSelected }) => {
  ...
  return (
    <div className="p-4 border rounded-lg shadow-md">
      ...
      <input
        type="checkbox"
        checked={isSelected}
        onChange={() => onToggleSelection(table.id)}
        className="mr-2"
      />
      <button
...
function UpdateTable() {
  ...
  // Real-time updates (polling for updated data every 5 seconds)
  useEffect(() => {
    const interval = setInterval(() => {
      // Simulate fetching updated table data
      // setTables(updatedTables);  // Uncomment to replace with real data fetch logic
  ...
  const markSelectedAsAvailable = () => {
    setTables((prevTables) =>
      prevTables.map((table) =>
        selectedTables.includes(table.id) ? { ...table, status: "available" } : table
      )
    );
    setSelectedTables([]);
  };
    ...
    <div className="p-4">
      ...
      <div className="flex flex-col gap-4 mb-4 sm:flex-row">
        ...
        <select
          ...
          className="p-2 border rounded-lg"
        >
          <option value="number">Sort by Table Number</option>
          <option value="status">Sort by Status</option>
          <option value="customerName">Sort by Customer Name</option>
        </select>
        <button
          onClick={markSelectedAsAvailable}
          className="px-4 py-2 text-white bg-green-600 rounded hover:bg-green-700"
        >
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {sortedTables.map((table) => (
          <TableCard
            ...
            onToggleSelection={handleTableSelection}
