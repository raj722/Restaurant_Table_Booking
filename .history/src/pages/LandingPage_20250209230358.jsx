...
const LandingPage = () => {
  ...
  useEffect(() => {
    const fetchUserRole = async () => {
      ...
      if (user) {
        const userRef = doc(db, "CustomerUser", user.uid); // Check if user is CustomerUser
        const userSnap = await getDoc(userRef);
        if (userSnap.exists()) {
          setUserRole("CustomerUser");
        } else {
          setUserRole("RestaurantUser"); // Otherwise, assume RestaurantUser
        }
      }
    };
  ...
  // Handle restaurant click based on user role
